...
void DGMassInverse_Direct::Setup()
{
   ...
   blocks_for_lumping.UseDevice(true);
   blocks_for_lumping = blocks;
   ...
   if (mode == BatchSolverMode::NATIVE || !Device::Allows(Backend::DEVICE_MASK))
   {
   ...
   else if (mode == BatchSolverMode::CUBLAS)
   {
      ...
      Vector tmp(blocks.Size(), mfem::MemoryType::DEVICE_UMPIRE_2);
      tmp.UseDevice(true);
      tmp = blocks;
      Array<double*> tmp_array(ne, mfem::MemoryType::DEVICE_UMPIRE_2);
      ...
      for (int i = 0; i < ne; ++i)
      {
        tmp_array.HostWrite()[i] = ptr_base + i*elem_dofs*elem_dofs;
      }
      ...
      for (int i = 0; i < ne; ++i)
      {
        matrix_array.HostWrite()[i] = ptr_base + i*elem_dofs*elem_dofs;
      }
...
void DGMassInverse_Direct::CopyElemMass(Vector &Me) const
{
  Me = blocks_for_lumping;
}

void DGMassInverse_Direct::LumpMatrix(Vector &lumpedM) const
{

  const double *d_elemMass = blocks_for_lumping.Read();
  double *d_lumpedM        = lumpedM.Write();

  const int order  = fes.GetFE(0)->GetOrder();
  const int NE     = fes.GetMesh()->GetNE();
  const int dim    = fes.GetMesh()->Dimension();
  const int dofs1D = order + 1;

  int totalDofs = 0;
  //mfem::asserta(dim == 1, "Direct solver only supports dim = 1, 2");
  if (dim == 2) totalDofs = dofs1D * dofs1D;
  if (dim == 3) totalDofs = dofs1D * dofs1D * dofs1D;
  auto massMat = mfem::Reshape(d_elemMass, totalDofs, totalDofs, NE);

  MFEM_FORALL(tid, totalDofs * NE, {
      const int r = tid % totalDofs;
      const int e = tid / totalDofs;

      {
        const int idx = r + e * totalDofs;
        double dot    = 0.0;

        for (int c = 0; c < totalDofs; ++c)
          {
            dot += massMat(r, c, e);  //mass matrix is symmetric...
          }
        d_lumpedM[idx] = dot;
      }
    });
